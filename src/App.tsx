...
const MOODS = {
  none: { label: "None", url: "" },
  piano: { label: "Soft Piano", url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3" }, // Placeholder
...
function AddItemModal({ type, onAdd, onClose }: any) {
  ...
  async function handleVideoFile(e: any) {
    ...
    // 15MB limit for direct JSON storage
...
function TuckedItem({ item, index = 0 }: any) {
  ...
  return (
    <div
      ...
      style={{ "--r": `${rot}deg`, "--y": `${yOff}px`, animationDelay: `${delay}ms` } as any}
    >
...
export default function App() {
  const [screen, setScreen] = useState("home"); // home | create | checkout | sealed | open | unwrap | view
  ...
  function resetAll() {
    ...
    try {
      ...
      // no addressable URL in this context
    }
  }
    ...
    <div className="app-root" style={{ background: currentTheme.bg, color: currentTheme.text } as any}>
      <style>{`
        ...
        .paper-card {
          ...
          border-radius: 4px;
          box-shadow: 0 2px 0 rgba(0,0,0,0.06), 0 18px 30px -18px rgba(35,25,10,0.55);
        ...
        .mini-caption {
          text-align: center; font-family: 'Special Elite', monospace; font-size: 11px;
          letter-spacing: 0.1em; color: var(--ink-soft); margin: 12px 0 2px;
        }
        ...
        /* ---------- buttons & inputs ---------- */
        .btn-primary, .btn-secondary, .btn-link, .btn-stamp, .btn-ghost-card {
          font-weight: 600; font-size: 14.5px; border-radius: 3px; cursor: pointer;
        ...
        .btn-stamp {
          font-family: 'Special Elite', monospace; font-weight: 700; font-size: 14.5px; letter-spacing: 0.02em;
          background: var(--paper); color: var(--stamp-red); border: 2px solid var(--stamp-red);
        ...
        .btn-ghost-card {
          font-family: 'Special Elite', monospace; font-size: 13.5px;
          background: var(--paper); color: var(--ink); border: 1.5px solid rgba(0,0,0,0.18); border-radius: 6px; padding: 12.5px 20px;
        }
        .btn-ghost-card:hover { border-color: var(--airmail); color: var(--airmail); }

        .text-input, .note-input, .handwritten-input, .code-input {
          width: 100%; font-family: 'Work Sans', sans-serif; font-size: 14.5px;
          border: 1.5px solid rgba(0,0,0,0.16); border-radius: 3px; padding: 11px 12px;
        ...
        .brand-mark {
          ...
          margin-bottom: 18px; box-shadow: 0 6px 16px -8px rgba(0,0,0,0.5);
        }
        ...
        .stamp-mark {
          position: absolute; top: 16px; right: 18px; font-family: 'Caveat', cursive; font-weight: 700;
          font-size: 15px; line-height: 1.25; text-align: center; color: var(--stamp-red);
          border: 2px solid var(--stamp-red); border-radius: 8px; padding: 6px 11px; transform: rotate(7deg); opacity: 0.88;
        }
        .stamp-badge {
          display: inline-block; font-family: 'Caveat', cursive; font-weight: 700; font-size: 21px;
          color: var(--stamp-red); border: 2px solid var(--stamp-red); border-radius: 8px; padding: 7px 16px;
          transform: rotate(-3deg); opacity: 0.9; margin: 8px 0 2px;
        }
        ...
        .item-icon-wrap {
          width: 60px; height: 60px; border-radius: 14px; background: linear-gradient(150deg, #fff, var(--paper-soft));
          display: flex; align-items: center; justify-content: center; color: var(--ink-soft);
          box-shadow: 0 10px 16px -10px rgba(30,20,5,0.5), 0 2px 3px rgba(0,0,0,0.08);
          transform: rotate(var(--rot, 0deg)); transition: transform 0.15s ease, color 0.15s ease;
        }
        ...
        .tucked-card { position: relative; background: var(--paper); padding: 14px; width: 210px; box-shadow: 0 10px 20px -10px rgba(0,0,0,0.5); border-radius: 2px; }
        @keyframes tucked-in {
          0% { opacity: 0; transform: translateY(calc(var(--y, 0px) + 16px)) rotate(var(--r)) scale(0.94); }
          100% { opacity: 1; transform: translateY(var(--y, 0px)) rotate(var(--r)) scale(1); }
        }
        .tucked-card { animation: tucked-in 0.5s ease both; }
        ...
        @keyframes pulse {
          0% { transform: scale(1); box-shadow: 0 0 0 0 rgba(163,57,47,0.4); }
          70% { transform: scale(1.05); box-shadow: 0 0 0 10px rgba(163,57,47,0); }
          100% { transform: scale(1); box-shadow: 0 0 0 0 rgba(163,57,47,0); }
        }
        @media (prefers-reduced-motion: reduce) {
          .btn-primary, .btn-stamp, .item-icon-wrap, .box-lid, .box-tape, .box-glow, .confetti-dot, .tucked-card, .vinyl { transition: none !important; animation: none !important; }
        }
      `}</style>
          ...
          <div className="workbench">
            <div className="label-card paper-card">
              ...
              <div className="label-body">
                ...
                <div className="field-row">
                  <span className="field-tag">To</span>
                  <input className="handwritten-input" value={to} onChange={(e) => setTo(e.target.value)} placeholder="cutie" maxLength={40} />
                </div>
                <div className="dashed-rule" />
                <div className="field-row">
                  <span className="field-tag">Lock until (optional)</span>
                  <input
                    type="datetime-local"
                    className="text-input"
                    style={{ background: 'transparent', border: 'none', borderBottom: '1.5px dashed rgba(0,0,0,0.3)', padding: '4px 0', fontSize: 13 }}
                    value={unlockDate}
                ...
                <div className="field-row">
                  <span className="field-tag">From</span>
                  <input className="handwritten-input" value={from} onChange={(e) => setFrom(e.target.value)} placeholder="you" maxLength={40} />
                </div>
                <div className="perforation" />
                <div className="barcode-wrap">
                  <Barcode seed={to + from + items.length} />
                  <p className="barcode-caption">
                    {items.length} item{items.length !== 1 ? "s" : ""} tucked in · not yet sealed
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="catalog-card paper-card paper-soft">
              <div className="panel-brand">
                <h2>A Little Box of Goodies</h2>
                <p className="sub">est. for sending a little care</p>
              </div>
              <div style={{ textAlign: "center" }}>
                <span className="stamp-badge">things to tuck inside</span>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center', marginTop: 10 }}>
                {Object.entries(THEMES).map(([key, t]) => (
                  <button
                    key={key}
                    className={`btn-secondary ${theme === key ? 'paper-soft' : ''}`}
                    ...
                    {items.map((it) => (
                      <div key={it.id} className="cart-row">
                        ...
                        <div className="cart-info">
                          <p className="cart-type">{(ITEM_TYPES as any)[it.type].label.toUpperCase()}</p>
                          <p className="cart-detail">{itemSummary(it)}</p>
                        </div>
                        <button className="remove-pill" onClick={() => removeItem(it.id)}>
                          remove
                        </button>
                      </div>
                    ))}
              ...
              <button className="btn-stamp" style={{ width: "100%", marginTop: 20 }} disabled={!canCheckout} onClick={() => setScreen("checkout")}>
        ...
        {screen === "checkout" && (
          <div className="workbench">
            ...
            <div className="catalog-card paper-card paper-soft checkout-card">
              <button className="btn-link back-edit" onClick={() => setScreen("create")}>
                <ArrowLeft size={13} style={{ marginRight: 4, verticalAlign: -2 }} />
                edit your box
              </button>
              <div className="panel-brand">
                <h2>A Little Box of Goodies</h2>
                <p className="sub">checkout · share your parcel</p>
              </div>
              <div>
                <span className="stamp-badge">send your package</span>
              </div>
              ...
              <button className="btn-stamp" style={{ width: "100%" }} disabled={paying || verifying} onClick={handlePay}>
                {verifying ? "Verifying Payment..." : paying ? "Processing…" : "Pay & Generate Link"}
              </button>
              <button className="btn-ghost-card" style={{ width: "100%", marginTop: 10 }} onClick={() => setPreviewOpen(true)}>
                Preview
              </button>
              ...
              <Barcode seed={to + from + items.length} />
              <p className="mini-caption">send it to someone special</p>
            </div>
          </div>
        )}
        ...
        {previewOpen && (
          <div className="modal-backdrop" onClick={() => setPreviewOpen(false)}>
            <div className="modal-card paper-card preview-modal" onClick={(e) => e.stopPropagation()}>
              <div className="modal-head">
                <h3>Preview</h3>
                <button className="icon-btn" onClick={() => setPreviewOpen(false)} aria-label="Close">
                  <X size={16} />
                </button>
              </div>
              <p className="hint" style={{ marginBottom: 14 }}>
                This is what {to || "they"} will see when they open it.
              </p>
              <div className="tucked-grid preview-grid">
                {items.map((it, idx) => (
                  <TuckedItem key={it.id} item={it} index={idx} />
                ))}
              </div>
            </div>
          </div>
        ...
        {screen === "sealed" && (
          <div className="workbench">
            ...
            <div className="catalog-card paper-card thankyou-card">
              ...
              <p className="barcode-caption" style={{ marginBottom: 18 }}>
                {barcodeSeed.replace(/(\d{4})(?=\d)/g, "$1 ")}
              </p>
              <h2 className="thankyou-title">Thank You For Caring</h2>
              <p className="thankyou-sub">have a nice day :)</p>

              <p className="eyebrow" style={{ textAlign: "center" }}>
                your link
              </p>
              <div className="link-box">{getShareUrl(shareCode)}</div>
              <button className="btn-stamp" style={{ width: "100%" }} onClick={copyLink}>
                {linkCopied ? (
                  <>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                ...
                />
                <button className="btn-primary" onClick={updateSecretWord} disabled={isSettingSecret || !secretWord.trim()}>
                  {isSettingSecret ? "Saving..." : "Set"}
                </button>
              </div>
              ...
              <button className="btn-secondary" style={{ width: "100%" }} onClick={copyCode}>
                {copied ? (
                  <>
                    Copied <Check size={14} />
                  </>
            ...
            <div className="kiosk-card paper-card">
              {needsSecret ? (
                <>
                  ...
                  />
                  <button className="btn-primary" onClick={() => loadPackage(enterCode)} disabled={openLoading || !enterSecret.trim()}>
                    {openLoading ? "Verifying..." : "Unlock Vault"}
                  </button>
                  <button className="btn-link" style={{ marginTop: 10 }} onClick={() => { setNeedsSecret(false); setEnterSecret(""); }}>
                    Try different code
                  </button>
                </>
              ) : (
                <>
                  <Mail size={26} strokeWidth={1.5} color="var(--airmail)" />
                  <h2>Open a package</h2>
                  ...
                  />
                  <button className="btn-primary" onClick={handleOpen} disabled={openLoading || !enterCode.trim()}>
                    {openLoading ? "Looking…" : (
                      <>
            ...
            <div className="box-stage">
              {isLocked ? (
                <div className="kiosk-card paper-card ethereal-glow">
                  ...
                  <p className="hint" style={{ margin: '15px 0' }}>
                    This package from <strong>{openedPackage.from}</strong> is sealed until:
                  </p>
                  <div className="code-display" style={{ fontSize: 18 }}>
                    {new Date(unlockTime!).toLocaleString()}
                  </div>
                  <p className="hint">Come back then to lift the lid!</p>
                  <button className="btn-secondary" style={{ marginTop: 20 }} onClick={resetAll}>
                    Go Back
                  </button>
                </div>
              ) : (
                  ...
                  <div className="box-visual">
                    ...
                    {currentTheme.stars && !lidUp && (
                      <div style={{ position: 'absolute', inset: 10, pointerEvents: 'none' }}>
                        {[...Array(6)].map((_, i) => (
                          <Sparkles key={i} size={12} color="#fff" style={{ position: 'absolute', left: `${Math.random() * 80 + 10}%`, top: `${Math.random() * 80 + 10}%`, opacity: 0.6 }} />
                        ))}
                      </div>
                    )}
                  </div>
            ...
            {viewMode === "classic" ? (
              <>
                <div className="view-label paper-card">
                  <span className="delivered-stamp">
                    <Check size={12} strokeWidth={3} /> delivered
                  </span>
                  <p className="eyebrow" style={{ textAlign: "center", marginBottom: 2 }}>
                    a package has arrived for
                  </p>
                  <p className="to-line">{openedPackage.to}</p>
                  <p className="from-line">
                    from {openedPackage.from}
                    {openedPackage.sealedAt
                      ? ` · ${new Date(openedPackage.sealedAt).toLocaleDateString(undefined, {
                          month: "short",
                          day: "numeric",
                          year: "numeric",
                        })}`
              ...
              <div className="constellation-view" style={{
                ...
                <div style={{ position: 'relative', zIndex: 10, width: '100%', display: 'flex', flexWrap: 'wrap', gap: 40, justifyContent: 'center', padding: 40 }}>
                  {openedPackage.items.map((it: any, idx: number) => {
                    const z = (idx % 3) * 20;
                    return (
                      <div
            ...
            <div className="view-footer">
              <div style={{ padding: '20px', background: 'rgba(255,255,255,0.1)', borderRadius: 12, marginTop: 20, textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginBottom: 15 }}>
                  <button className="btn-secondary" style={{ borderRadius: '50%', width: 50, height: 50, padding: 0 }} onClick={() => sendReaction('heart')}>
                    ❤️
                  </button>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  />
                  <button className="btn-primary" onClick={handleSendReaction} disabled={sendingReaction || !reactionText.trim()}>
                    Send
                  </button>
                </div>
              </div>
              ...
              <div className="thin-rule" />
              <p className="mini-caption" style={{ marginTop: 4 }}>
                sent with care via a little box of goodies
              </p>
